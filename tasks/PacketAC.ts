...
task("deployPacketAC", "Deploy Packet")
        ...
        if (NOT_PROXY) {
            ...
            const packet = await hre.upgrades.deployProxy(
                ...
                [
                    taskArgs.chain,
                    taskArgs.relaychain,
                    String(CLIENT_MANAGER_AC_ADDRESS),
            ...
            console.log("export PACKET_AC_ADDRESS=%s", packet.address.toLocaleLowerCase())
            fs.appendFileSync('env.txt', 'export PACKET_AC_ADDRESS=' + packet.address.toLocaleLowerCase() + '\n')
        }
    })
...
task("checkRouting", "query")
    .setAction(async (taskArgs, hre) => {
        const packetFactory = await hre.ethers.getContractFactory('PacketAC')
        const packet = await packetFactory.attach(String(PACKET_AC_ADDRESS))
...
task("initPacket", "set endpoint and execute contract addresses")
    .setAction(async (taskArgs, hre) => {
        ...
        let packetRec = await packet.initEndpoint(String(ENDPOINT_AC_ADDRESS), String(EXECUTE_ADDRESS))
