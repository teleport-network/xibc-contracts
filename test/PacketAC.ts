...
import { MockEndpointAC, Execute, PacketAC, ClientManagerAC, MockTendermint, AccessManager, ERC20 } from '../typechain'
...
describe('Packet', () => {
    ...
    it("upgrade packet", async () => {
        const mockPacketFactory = await ethers.getContractFactory("MockPacketAC")
    ...
    const deployEndpoint = async () => {
        const endpointFactory = await ethers.getContractFactory('MockEndpointAC', accounts[0])
