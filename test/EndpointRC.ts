...
import { EndpointRC, Execute, PacketRC, ClientManagerRC, MockTendermint, AccessManager, ERC20 } from '../typechain'
...
describe('EndpointRC', () => {
    ...
    it("bind token", async () => {
        ...
        await endpoint.bindToken(tokenAddress, bindOriToken, bindDstChain, 0)
        let bind = await endpoint.getBindings(tokenAddress + "/" + bindDstChain)
        expect(bind.bound).to.eq(true)
        expect(bind.oriChain).to.eq(bindDstChain)
        expect(bind.oriToken).to.eq(bindOriToken)
        expect(bind.scale).to.eq(0)

        await endpoint.bindToken(tokenAddress, bindOriToken, reBindDstChain, 1)
        bind = await endpoint.getBindings(tokenAddress + "/" + reBindDstChain)
        expect(bind.bound).to.eq(true)
        expect(bind.oriChain).to.eq(reBindDstChain)
        expect(bind.oriToken).to.eq(bindOriToken)
        expect(bind.scale).to.eq(1)

        await endpoint.bindToken(tokenAddress, reBindOriToken, bindDstChain, 3)
        bind = await endpoint.getBindings(tokenAddress + "/" + bindDstChain)
        expect(bind.bound).to.eq(true)
        expect(bind.oriChain).to.eq(bindDstChain)
        expect(bind.oriToken).to.eq(reBindOriToken)
        expect(bind.scale).to.eq(3)
    ...
    it("upgrade endpoint", async () => {
        ...
        const mockEndpointFactory = await ethers.getContractFactory("MockEndpointRC")
        ...
        let binds = await upgradedEndpoint.bindings(erc20Contract.address.toLocaleLowerCase() + "/" + packet.srcChain)
        let totalSupply = (await erc20Contract.totalSupply()).toString()
        
        expect(binds.oriChain).to.eq(packet.srcChain)
        expect(binds.oriToken).to.eq(transferData.token)
        expect(binds.scale).to.eq(0)
    ...
    const deployTendermint = async () => {
        ...
        let teleportClient = await clientManager.clients(testChainName)
        expect(teleportClient).to.eq(tendermint.address)

        let latestHeight = await clientManager.getLatestHeight(testChainName)
