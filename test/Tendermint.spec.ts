...
import { ClientManagerAC, Tendermint, AccessManager } from '../typechain'
...
describe('Tendermint', () => {
    ...
    it("test updateClient ", async () => {
        ...
        result = await clientManagerAC.updateClient(headerBz)
        await result.wait()
    ...
    const createClient = async function (lightClientAddress: any, clientState: any, consensusState: any) {
        ...
        await clientManagerAC.createClient(lightClientAddress, clientStateBuf, consensusStateBuf)
    }
    ...
    const initialize = async () => {
        ...
        let latestHeight = await clientManagerAC.getLatestHeight()
