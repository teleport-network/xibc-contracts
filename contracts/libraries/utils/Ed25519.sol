...
library Ed25519 {
    ...
    function verify(
        ...
        {
            bytes memory rs = new bytes(k.length + r.length + m1.length);
            ...
            for (uint256 i = 0; i < m1.length; i++) {
                rs[i + 64] = m1[i];
            }
            uint64[8] memory result = Sha512.hash(rs);

            uint256 h0 = uint256(result[0]) |
                (uint256(result[1]) << 64) |
                (uint256(result[2]) << 128) |
            ...
            uint256 h1 = uint256(result[4]) |
                (uint256(result[5]) << 64) |
                (uint256(result[6]) << 128) |
