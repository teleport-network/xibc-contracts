...
contract AccessManager is AccessControlUpgradeable {
    ...
    function initialize(address _multiSignWallet) public initializer {
        ...
        // clientManager
        _setupRole(CREATE_CLIENT_ROLE, multiSignWallet);
        _setupRole(UPGRADE_CLIENT_ROLE, multiSignWallet);
        _setupRole(REGISTER_RELAYER_ROLE, multiSignWallet);

        // packet
        _setupRole(PAUSER_ROLE, multiSignWallet);

        // endpoint
        _setupRole(BIND_TOKEN_ROLE, multiSignWallet);
    }
