...
library ValidatorSet {
    ...
    function _decode(
        ...
        while (pointer < offset + sz) {
            ...
            if (fieldId == 1) {
                ...
                pointer += _read_total_voting_power(
                    pointer,
                    bs,
                    nil(),
                    counters
    ...
    function _decode_Validator(uint256 p, bytes memory bs)
        internal
        pure
        returns (Validator.Data memory, uint256)
    {
        uint256 pointer = p;
    ...
    function _encode(
        ...
        if (r.validators.length != 0) {
            for (i = 0; i < r.validators.length; i++) {
                ...
                pointer += Validator._encode_nested(
                    r.validators[i],
                    pointer,
                    bs
                );
        ...
        for (i = 0; i < r.validators.length; i++) {
            e +=
                ...
                ProtoBufRuntime._sz_lendelim(
                    Validator._estimate(r.validators[i])
                );
        }
     ...
     */
    function addValidators(Data memory self, Validator.Data memory value)
        internal
        pure
    {
        /**
         ...
         */
        Validator.Data[] memory tmp = new Validator.Data[](
...
library Validator {
    ...
    function _decode(
        ...
        while (pointer < offset + sz) {
            ...
            pointer += bytesRead;
            if (fieldId == 1) {
                pointer += _read_addr(pointer, bs, r, counters);
            } else if (fieldId == 2) {
    ...
    function _decode_PublicKey(uint256 p, bytes memory bs)
        internal
        pure
        returns (PublicKey.Data memory, uint256)
    {
        uint256 pointer = p;
        (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(
            pointer,
